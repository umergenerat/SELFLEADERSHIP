...
export default function App() {
  ...
  const { profile: student, toggleHealthAlert } = useApp();
  ...
  return (
    <div className="flex h-screen w-full p-2 md:p-4 lg:p-6 gap-2 md:gap-4 lg:gap-6 text-white font-sans overflow-hidden bg-slate-900" dir="rtl">
      ...
      `}>
        <div className="glass p-6 flex flex-col items-center gap-4 text-center mt-8 lg:mt-0 relative">
          <button 
            onClick={() => setIsSidebarOpen(false)}
            className="absolute top-2 right-2 lg:hidden text-slate-400 hover:text-white"
          >
            <X size={24} />
          </button>
          <div className="w-20 h-20 rounded-full border-4 border-emerald-500 p-1">
          ...
          <div>
             <h2 className="text-white font-bold">{student.name}</h2>
             <p className="text-emerald-400 text-xs">{student.level}</p>
          </div>
        </div>

        <nav className="glass flex-1 py-4 overflow-y-auto flex flex-col">
          {navItems.map((item) => {
      ...
      <main className="flex-1 flex flex-col gap-4 lg:gap-6 overflow-hidden relative">
        {/* Top Navbar */}
        <header className="glass px-4 md:px-8 py-4 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-lg md:text-xl font-bold text-white">مسار الريادة الذكي</h1>
              <p className="text-[10px] md:text-xs text-slate-400">دفتر القيادة والتتبع التربوي</p>
            </div>
          </div>
